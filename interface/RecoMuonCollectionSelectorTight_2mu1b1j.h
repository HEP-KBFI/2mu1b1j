...
public:
  ...
  RecoMuonSelectorTight_2mu1b1j(int  era,
                                int  index = -1,
                                bool debug = false);
...
public:
  ...
  RecoMuonCollectionSelectorTight_2mu1b1j(int  era,
                                          int  index = -1,
                                          bool debug = false)
    : selIndex_(index)
  ...
  std::vector<const RecoMuon *>operator()(
    const std::vector<const RecoMuon *>& muons) const
  {
    std::vector<const RecoMuon *> selMuons;
    int idx = 0;

    for (typename std::vector<const RecoMuon *>::const_iterator muon =
           muons.begin();
         muon != muons.end(); ++muon) {
      if (selector_(**muon)) {
