...
class analyzeConfig_2mu1b1j(analyzeConfig):
  ...
  def __init__(self, outputDir, executable_analyze, lepton_selections, hadTau_selection, central_or_shifts,
    ...
    self.cfgFile_analyze_original = os.path.join(self.workingDir, "analyze_2mu1b1j_cfg.py")
  ...
  def createCfg_analyze(self, inputFiles, outputFile, sample_category, triggers, lepton_selection, hadTau_selection,
    ...
    lines.append("process.analyze_2mu1b1j.use_triggers_1e = cms.bool(%s)" % ("1e" in triggers))
    lines.append("process.analyze_2mu1b1j.use_triggers_2e = cms.bool(%s)" % ("2e" in triggers))
    ...
    lines.append("process.analyze_2mu1b1j.use_triggers_1e1mu = cms.bool(%s)" % ("1e1mu" in triggers))
    lines.append("process.analyze_2mu1b1j.leptonSelection = cms.string('%s')" % lepton_selection)
    lines.append("process.analyze_2mu1b1j.hadTauSelection = cms.string('%s')" % hadTau_selection)
